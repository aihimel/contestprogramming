...
int main(){
	...
	int long long x, y, x_stone, y_stone, stone;
	
	//~ ifstream input;
	//~ input.open("input.txt");
	
	for(IN >> x >> y >> stone; !IN.eof(); IN >> x >> y >> stone){
		
		for(x_stone = 0; x > 0; x -= stone) x_stone++;
		for(y_stone = 0; y > 0; y -= stone) y_stone++;
		cout << x_stone * y_stone << endl;
