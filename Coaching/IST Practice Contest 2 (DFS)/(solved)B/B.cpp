...
int main(){
	...
	for(IN >> NODE; !IN.eof(); IN >> NODE){
		...
		for(i = 1; i <= NODE; i++) IN >> DATA[i];
		
		for(i = 1, max_height = 1; i <= NODE; i++) 
			if(DATA[i] == -1) max_height = max(max_height, dfs(i));
...
int dfs(int node){
	...
	for(i = 1, height = 0; i <= NODE; i++)
		if(DATA[i] == node) height = max(height, dfs(i));
