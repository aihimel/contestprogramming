...
int main(){
	...
	int long long length, primary_position, secondary_position, days, i, beg, end;
	char data[2*10000000], temp;
	...
	for(IN >> data; !IN.eof(); IN >> data){
		...
		IN >> days;
		for(i = 0; i < days; i++){
			IN >> primary_position; secondary_position = length - (primary_position - 1);
			primary_position--; secondary_position--; // Decreasing by one.
			
			// Reversing the string
			for(beg = primary_position, end= secondary_position; beg < end; beg++, end--){
				temp = data[beg];
				data[beg] = data[end];
				data[end] = temp;
			}
