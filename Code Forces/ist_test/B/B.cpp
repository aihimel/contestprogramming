...
int main(){
	...
	int v1, v2, t, d, i, result;
	
	//~ ifstream input;
	//~ input.open("input.txt");
	
	for(IN >> v1 >> v2 >> t >> d; !IN.eof(); IN >> v1 >> v2 >> t >> d){
		
		int data[t];
		for(i = 1, data[0] = v1; i < t; i++) data[i] = data[i-1] + d;
		
		for(i = t - 1, data[t-1] = v2; i > 0; i--){
			if( abs( data[i-1] - data[i] ) > d ) data[i-1] = data[i] + d;
				else break;
		}
		
		for(i = result = 0; i < t; i++) result += data[i];
		cout << result << endl;
